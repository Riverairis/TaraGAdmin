...
const Login = ({ onLoginSuccess }) => {
  ...
  try {
    const saved = localStorage.getItem('tg-dark');
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('tg-dark', String(darkMode));
    } catch {}
  }, [darkMode]);

  const toggleDarkMode = () => setDarkMode(prev => !prev);

  // Use the correct API base URL for your setup
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call your backend login endpoint with full URL
      ...
      if (err.message.includes('Failed to fetch')) {
        setError('Network error: Please check your connection and make sure the server is running on port 5000.');
      } else if (err.message.includes('No response')) {
        setError('Server is not responding. Please make sure your backend server is running.');
      } else {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/auth/send-password-reset`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
              ...
              <img 
                          src={newLogo} 
                          alt="TaraG Logo" 
                          className="w-20 h-50 rounded-lg mr-3"
                        />
            
          </div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">TaraG Admin</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-2">Sign in to access the administrator panel</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 relative">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-3 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                  placeholder="admin@tarag.com"
                />
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="block w-full pl-10 pr-10 py-3 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-gradient-to-r from-cyan-500 to-cyan-500 hover:from-cyan-500 hover:to-teal-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 transition-all duration-300 ${isLoading ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {isLoading ? (
                <>
        ...
        <div className="text-center mt-8">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            &copy; {new Date().getFullYear()} TaraG. All rights reserved.
          </p>
        </div>
      </div>
    </div>
